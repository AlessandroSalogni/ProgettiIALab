...
public class ProbabilityTable implements CategoricalDistribution, Factor {
	...
	private Map<RandomVariable, Object>[] bestVars;
	//
	...
	public ProbabilityTable(double[] vals, RandomVariable... vars) {
		...
		bestVars = new LinkedHashMap[vals.length];
		for(int i = 0; i < bestVars.length ; i++)
			bestVars[i] = new LinkedHashMap<>(i);
	...
	public Map<RandomVariable, Object>[] getBestVarsValues() {
		return bestVars;
	}
	...
	@Override
	public Factor maxOut(RandomVariable maxOutVar) {
		Set<RandomVariable> randomVariables = new LinkedHashSet(this.randomVarInfo.keySet());
		randomVariables.remove(maxOutVar);

		final ProbabilityTable maxedOut = new ProbabilityTable(randomVariables);
		if (maxedOut.getValues().length == 1) {
			for (int i=0; i < getValues().length; i++)
				if (maxedOut.getValues()[0] < getValues()[i]) {
					maxedOut.getValues()[0] = getValues()[i];

					maxedOut.bestVars[0] = new LinkedHashMap<>(bestVars[i]);
					maxedOut.bestVars[0].put(maxOutVar, randomVarInfo.get(maxOutVar).getDomainValueAt(i));
				}
		}
		else {
			final Object[] term1Values = new Object[maxedOut.randomVarInfo.size()];
			final Object[] term2Values = new Object[randomVarInfo.size()];
			ProbabilityTable.Iterator di = (possibleWorld, probability) -> {
				int i = 0;
				for (RandomVariable rv : maxedOut.randomVarInfo.keySet()) {
					term1Values[i] = possibleWorld.get(rv);
					i++;
				}

				i=0;
				for (RandomVariable rv : randomVarInfo.keySet()) {
					term2Values[i] = possibleWorld.get(rv);
					i++;
				}

				double[] values = maxedOut.getValues();
				int index = maxedOut.getIndex(term1Values);

				if(values[index] < probability) {
					values[index] = probability;

					maxedOut.bestVars[index] = new LinkedHashMap<>(bestVars[getIndex(term2Values)]);
					maxedOut.bestVars[index].put(maxOutVar, possibleWorld.get(maxOutVar));
				}
			};
	...
	public ProbabilityTable pointwiseProduct(final ProbabilityTable multiplier) {
		Set<RandomVariable> prodVars = SetOps.union(randomVarInfo.keySet(), multiplier.randomVarInfo.keySet());
		return pointwiseProductPOS(multiplier, prodVars.toArray(new RandomVariable[prodVars.size()]));
	}
	...
	public ProbabilityTable pointwiseProductPOS(final ProbabilityTable multiplier, RandomVariable... prodVarOrder) {
		...
		if (!product.randomVarInfo.keySet().equals(SetOps.union(randomVarInfo.keySet(), multiplier.randomVarInfo.keySet()))) {
		...
		if (1 == product.getValues().length) {
			...
			ProbabilityTable.Iterator di = new ProbabilityTable.Iterator() {
				...
				public void iterate(Map<RandomVariable, Object> possibleWorld, double probability) {
					int term1Idx = termIdx(term1Values, ProbabilityTable.this, possibleWorld);
					...
					product.bestVars[idx] = new LinkedHashMap<>(bestVars[term1Idx]);
					product.bestVars[idx].putAll(multiplier.bestVars[term2Idx]);
