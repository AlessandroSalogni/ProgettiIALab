kalmanFilter = function(F, H, cov0, covX, covZ, covT, iteration) {
  ...
  for(i in 1 : (nrow(x)-1)) {
    muF = F%*%x[i,]
    transNoise = abs(mvrnorm(1, muF, covX) - muF) ##errore di transizione centrato in 0 per ogni variabile
    x[i+1,] = muF + transNoise ##stato successivo = muF + errore di transizione
    
    muH = H%*%x[i+1,]
    obsNoise = abs(mvrnorm(1, muH, covZ) - muH) ##errore di misurazione centrato in 0 per ogni variabile
    z[i+1,] = muH + obsNoise ##osservazione sullo stato successivo = muH + errore di misurazione
    
    p = F*covT*t(F) + covX
    kalmanGain = p*t(H)*inv(H*p*t(H) + covZ)
    kalmanGains[[i]] = kalmanGain
    covT = (diag(dim(covT)[1]) - kalmanGain*H)*p
    t[i+1,] = F%*%t[i,] + kalmanGain%*%(z[i+1,] - H%*%F%*%t[i,])
  }
  
  return(list("real" = x, "observed" = z, "kalman" = t, "kalmanGains" = kalmanGains))
}
