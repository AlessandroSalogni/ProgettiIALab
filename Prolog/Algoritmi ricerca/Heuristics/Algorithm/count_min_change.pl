...
cambi_in_ground_heuristic([at(SP),in(Linea,_)], [at(SA),_], Costo) :-
  findall(sali(LineaPartenza, 0), applicabile(sali(LineaPartenza, 0), [at(SA),ground]), ListaLineeArrivo),
  findall(sali(LineaArrivo, 0), applicabile(sali(LineaArrivo, 0), [at(SP),ground]), ListaLineePartenza),
...
calcola_min_cambi_in_ground([sali(Linea,_)], [sali(Linea,_)|_], 0) :- !.
...
calcola_min_cambi_in_ground([sali(Linea,_)|_], [sali(Linea,_)|_], 0) :- !.
