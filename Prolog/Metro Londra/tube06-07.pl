...
costo(vai(_,_,SP,SA), Costo) :-
  stazione(SP, SPx, SPy),
  stazione(SA, SAx, SAy),
  Temp is ((SPx - SAx) ^ 2) + ((SPy - SAy) ^ 2),
  Distanza is sqrt(Temp),
  Costo is (Distanza * 625 / 10) / 60. % Sistemare % 10m/s
...
iniziale([at('London Bridge'),ground]).
finale([at('Earls Court'),ground]).
